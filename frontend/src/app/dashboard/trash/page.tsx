...
import {
    ...
    ArrowLeft, Calendar, Clock, ArrowUpDown,
} from "lucide-react";
...
import { LayoutToggle } from "@/features/dashboard/components/LayoutToggle";
import { useTrashPreferences } from "@/features/dashboard/hooks/useTrashPreferences";
...
import { formatDate, tsToMs } from "@/features/dashboard/utils";
...
const dict = {
    ...
    en: {
        ...
        confirmBulkDelete: (n: number) =>
            `[WARNING] Permanently delete ${n} items?\nThis cannot be undone.`,
    },
} as const;
...
export default function TrashPage() {
    ...
    const { prefs, update } = useTrashPreferences();
    const { sortKey, sortDir, layout } = prefs;
    ...
    const handleBulkRestore = async () => {
        ...
        try {
            await Promise.all(
                Array.from(selectedIds).map((id) => workspaceService.restoreWorkspace(id)),
            );
            clearSelection();
            await fetchTrash();
        } catch (error) {
    ...
    const handleRestore = async (ws: WorkspaceData) => {
        if (!confirm(tx.confirmRestore)) return;
        await workspaceService.restoreWorkspace(ws.id);
        fetchTrash();
    };

    const handlePermanentDelete = async (ws: WorkspaceData) => {
        if (!confirm(tx.confirmDelete)) return;
        await workspaceService.permanentDeleteWorkspace(ws.id);
        fetchTrash();
    };
                ...
                <header className="h-16 border-b flex items-center justify-between px-8 shrink-0">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 rounded-lg border bg-slate-50 p-1 text-xs shrink-0">
                            ...
                            <select
                                value={sortKey}
                                onChange={(e) => update("sortKey", e.target.value as TrashSortKey)}
                        ...
                        {isSelectionMode ? (
                            <div className="flex items-center gap-3 animate-in slide-in-from-right-4 duration-200">
                                ...
                                >
                                    ...
                                    {isProcessing ? tx.restoring : tx.restore}
                                </Button>
                                <Button
                                ...
                                <div className="bg-white rounded-2xl border divide-y overflow-hidden shadow-sm">
                                    {filteredItems.map((ws) => (
                                        <TrashRow
                                            ...
                                            onSelect={() =>
                                                isSelectionMode ? toggleSelect(ws.id) : null
                                            }
                                            onRestore={() => handleRestore(ws)}
                                            onDelete={() => handlePermanentDelete(ws)}
                                        />
                                    ))}
                                </div>
                            ) : (
...
function TrashRow({ ws, isSelectionMode, isSelected, onSelect, onRestore, onDelete, tx }: ItemProps) {
            ...
            {!isSelectionMode && (
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Button
                        ...
                        className="text-red-500 hover:bg-red-50 h-8 px-3"
                    >
...
function TrashCard({ ws, isSelectionMode, isSelected, onSelect, onRestore, onDelete, tx }: ItemProps) {
    const createdAt = formatDate(ws.createdAt);
    const deletedAt = formatDate(ws.deletedAt);
    ...
    return (
        <div
            onClick={onSelect}
            className={cn(
                "group flex flex-col rounded-xl border bg-white p-4 transition-colors shadow-sm cursor-pointer",
            ...
            <div className={cn("mt-3 flex flex-col gap-1", isSelectionMode ? "pl-8" : "pl-12")}>
                ...
                {deletedAt && (
                    <div className="flex items-center gap-1.5 text-[11px] text-red-400">
                        <Clock className="h-3 w-3 shrink-0" />
                        <span>{tx.deletedLabel} {deletedAt}</span>
                    </div>
                )}
            </div>
            {!isSelectionMode && (
