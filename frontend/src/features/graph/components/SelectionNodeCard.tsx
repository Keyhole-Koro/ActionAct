...
export type SelectionNode = Node<SelectionNodeData, 'selectionNode'>;

export function SelectionNodeCard({ id, data, isConnectable }: NodeProps<SelectionNode>) {
    const { toggleOptionSelection } = useAgentInteractionStore();
    const isReadOnly = data.groupStatus !== 'pending';

    const handleSelect = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (!isReadOnly) {
            toggleOptionSelection(data.groupId, data.option_id);
        }
    };
    ...
    if (isReadOnly) {
        if (data.groupStatus === 'selected' && data.selected) {
    ...
    return (
        <>
            ...
            <Card className={`${baseClasses} ${stateClasses}`} onClick={handleSelect}>
                <CardHeader className="p-3 pb-2 flex flex-row items-center justify-between gap-2">
                    <CardTitle className="text-sm font-medium leading-tight">
                        {data.label}
                    </CardTitle>
                    {data.selected && <Badge variant="default" className="text-[10px] px-1 py-0 bg-amber-600 hover:bg-amber-600">Selected</Badge>}
                    {!data.selected && <Badge variant="outline" className="text-[10px] px-1 py-0 text-amber-700 border-amber-200">Select</Badge>}
                </CardHeader>
                {data.content_md && (
                ...
                <CardFooter className="p-3 pt-0 flex justify-end">
                    <div className="flex items-center text-xs text-amber-700 font-medium gap-1">
                        {data.selectionMode === 'single' ? (
                            data.selected ? <Circle className="w-4 h-4 fill-amber-600 text-amber-600" /> : <Circle className="w-4 h-4" />
                        ) : (
                            data.selected ? <Check className="w-4 h-4" /> : <div className="w-4 h-4 border rounded-sm border-amber-400" />
                        )}
                        {!isReadOnly && <span className="ml-1 opacity-70">Click to {data.selected ? 'deselect' : 'select'}</span>}
                    </div>
                </CardFooter>
            </Card>
