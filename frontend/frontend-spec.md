...
* node click は node を active にするが、right panel は開かない
* right panel は node card 内の `Details` control を押したときだけ開く
...
* `node-detail`
  ...
  * ACT node の場合は `referencedNodeIds` を関連 node の lightweight source として利用してよい
  * `persisted-node` detail は evidence を開いたときに追加購読してよい
...
* `src/config/local.json` の `useMocks=true` のとき **完全モックモード**
...
* `src/config/prod.json` の `useMocks=false` のとき **本番モード**（Firebase + Connect RPC）
...
* detail pane の本文表示は store snapshot を使う
* evidence は tree 購読 payload に含めず、detail pane を開いたときだけ購読する
